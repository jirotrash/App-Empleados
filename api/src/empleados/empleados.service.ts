...
export class EmpleadosService {
    ...
    async getReporteProduccion( id_empleado: number, fechaInicio: string, fechaFin: string ){
        // Return aggregated production sums per day to avoid huge raw payloads.
        // Accept empty id_empleado (all employees). If no fechaInicio/fechaFin provided, default to last 12 months.
        const now = new Date();
        const endDate = new Date(now.getFullYear(), now.getMonth() + 1, 0); // last day of current month
        const startDate = new Date(endDate.getFullYear(), endDate.getMonth() - 11, 1); // first day, 11 months before

        const inicioISO = fechaInicio
            ? // expect incoming in yyyy-mm-dd or dd/mm/yyyy; try to normalize
              (fechaInicio.includes('/') ? (() => { const [d,m,y]=fechaInicio.split('/'); return `${y}-${m.padStart(2,'0')}-${d.padStart(2,'0')}`; })() : fechaInicio)
            : startDate.toISOString().slice(0,10);

        const finISO = fechaFin
            ? (fechaFin.includes('/') ? (() => { const [d,m,y]=fechaFin.split('/'); return `${y}-${m.padStart(2,'0')}-${d.padStart(2,'0')}`; })() : fechaFin)
            : endDate.toISOString().slice(0,10);

        const qb = this.repoProduccion
            .createQueryBuilder("p")
            .leftJoin("p.empleado", "e")
            .select(["to_char(p.fecha, 'YYYY-MM-DD') as fecha", "SUM(p.unidadesProducidas) as unidades"])
            .andWhere("p.fecha BETWEEN :inicio AND :fin", { inicio: inicioISO, fin: finISO });

        if (id_empleado) {
            qb.andWhere("p.id_empleado = :id", { id: id_empleado });
        }

        const rows = await qb.groupBy("to_char(p.fecha, 'YYYY-MM-DD')").orderBy('fecha','ASC').getRawMany();
        // rows: [{ fecha: '2025-01-01', unidades: '123' }, ...]
        return rows.map(r => ({ fecha: r.fecha, unidades: Number(r.unidades) }));
    }
